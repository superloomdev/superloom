...
Pick the five-to-seven themes that apply to the module's class. The principle behind these bullets lives in [`project-structure.md`](project-structure.md#coding-practices) - "All external libraries wrapped".
...
Every module belongs to one of eight classes (enumerated in [`module-classes.md`](module-classes.md)). Each class can add one section between sections 4 (Why bullets) and 7 (Aligned with Superloom). The class-specific section sits at position 6 in the section order.
...
The canonical reasoning, audience map, and per-class footprint are documented in [`module-classes.md` → Universal Documentation Footprint](module-classes.md#universal-documentation-footprint).
...
*Note: cloud-only NoSQL services (e.g. DynamoDB) are Class D, not Class C. The dividing line is whether the database can be self-hosted on commodity infrastructure. See [`module-classes.md`](module-classes.md#class-d-cloud-service-wrapper).*
...
**`docs/` folder shape** per [`module-docs-complex.md`](module-docs-complex.md):
...
Cannot function on its own. Implements a Class E parent module's adapter contract for a single backend or runtime. For the full conceptual definition see [`module-classes.md` → Class F](module-classes.md#class-f-dependent-adapter).

Class F has two subtypes: **stores** (`-store-[backend]`, data persistence) and **adapters** (`-adapter-[name]`, everything else: runtimes, transports, integrations). Both subtypes share the standard injected-Lib factory shape (loader takes `(shared_libs, config)`, companion files, factory `createInterface`). See `module-structure.md` → "Storage Adapter Skeleton" and "Adapter Skeleton" for the code templates.
...
The full prose-style guide lives in [`documentation-authoring.md`](../../principles/documentation-authoring.md). The spelling and prose-quality table lives in [`code-formatting.md`](code-formatting.md#spelling-and-prose-quality). Every module `README.md`, every `docs/*.md` file, and every `ROBOTS.md` follows those rules. The list below is a fast reference for the rules most often violated when authoring new module docs. It is not a replacement for the writing guide.
...
The full ban-list lives in [`documentation-authoring.md`](../../principles/documentation-authoring.md#banned-vocabulary). The most-violated entries while authoring module docs are: `facilitate`, `leverage`, `utilize`, `comprehensive`, `robust`, `streamline`, `it is worth noting`, `this ensures that`, `in order to`. Every one of these has a shorter plain replacement.

### American English

All project text uses American English (`-ize` not `-ise`, `-or` not `-our`, `license` not `licence`). The full table lives in [`documentation-authoring.md`](../../principles/documentation-authoring.md#prose-mechanics).
...
- **Em dashes (`—`) anywhere in the module docs.** Tell-tale sign of AI-generated prose. Use the patterns in [Writing Style and Prose Quality](#writing-style-and-prose-quality). Source of truth: [`documentation-authoring.md`](../../principles/documentation-authoring.md).
...
- [`documentation-authoring.md`](../../principles/documentation-authoring.md) is the canonical writing guide. It defines the human-first prose style, the em-dash ban, the AI-sounding-phrase ban-list, the table-cell-period rule, and the spelling and sentence-length rules that every module README and `docs/*.md` file follows.
- [`code-formatting.md`](code-formatting.md#spelling-and-prose-quality) holds the spelling and prose-quality table that applies to every file the project ships, including `.js` comments, `.md` docs, `package.json`, `ROBOTS.md`, and commit messages.
- [`module-classes.md`](module-classes.md) lists the six module classes and which class each existing module belongs to.
- [`module-docs-complex.md`](module-docs-complex.md) is the deep guide for `docs/` folders in Class E feature modules.
- **Pilot module READMEs** are the canonical starting templates. Copy the closest pilot for the class you are migrating: `js-server-helper-sql-mysql/` for Class C SQL (server-required), `js-server-helper-sql-sqlite/` for Class C SQL (embedded), `js-server-helper-nosql-mongodb/` for Class C NoSQL, `js-server-helper-nosql-aws-dynamodb/` for Class C/D cloud-managed NoSQL, `js-server-helper-storage-aws-s3/` for Class D cloud storage. See [Class-Specific Templates and Reusable Wording](#class-specific-templates-and-reusable-wording) for which pilot to copy per class.
- [`project-structure.md`](project-structure.md#coding-practices) - the "All external libraries wrapped" principle that the value bullets articulate
