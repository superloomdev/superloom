...
- Reference any [platform identifier](project-structure.md#platform-identifiers) (`web`, `rw`, `rn`, `ios`, `and`)
...
If a helper behaves differently across platforms, it **does not belong** in `catalog-core`.
...
- Foundation modules (`js-helper-utils`, `js-helper-debug`) must never depend on each other or on any other helper - see [`dependencies.md`](dependencies.md)
...
- [Server Helper Modules](catalog-server.md) - server-only helpers (DB drivers, cloud SDKs)
- [Module Structure (JavaScript)](module-structure) - the factory pattern every helper module follows
- [Peer Dependencies](dependencies.md) - how foundation modules stay self-contained
