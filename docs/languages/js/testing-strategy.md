...
The shape of `_test/package.json` (private flag, `file:../` self-reference, registry semver for shared helpers, required scripts) is documented in [`publishing.md` → Test Directory Structure](publishing.md#test-directory-structure). That is the canonical home; do not duplicate the example here.
...
Modules that use the [Adapter Pattern](module-structure#adapter-pattern-multi-backend-helper-modules) - a parent module plus N standalone backend packages - test in three tiers. Each tier runs at a different layer and answers a different question.
...
- **A smoke suite that walks a component/plugin/handler roster runs with the target library's strictest contract mode enabled.** For themed components, undeclared utility/token reads throw during the roster walk, so a dead reference surfaces as a test failure rather than silently resolving to `undefined`. Lenient roster rendering proves reachability only; it cannot prove styling correctness. If no strict mode exists, add a static contract audit that compares every literal reference against the generated registry; the audit must fail on any literal key the runtime registry does not declare.
- **Generated reference theme packages record source provenance and verify it at test time.** A generated package (e.g., Material, Carbon) records the base package version, base distribution shasum, and generator schema revision in its metadata. Tests fail when the installed base shasum does not match the registry shasum, when provenance is stale or missing, or when regeneration into a temporary directory is not byte-identical to the committed data files. Exact type-role values (size, line height, weight, tracking, family) are package contract tests asserted with `deepEqual`. After a same-version base republish, every downstream generated package is regenerated and republished, and every consumer lockfile is refreshed. See [Theming - Generated artifact provenance](client/theming.md#generated-artifact-provenance).

---

## Further Reading

- [Unit Test Authoring (JavaScript)](unit-test-authoring.md) - how to write a single unit test
...
- [Module README Structure](module-docs.md) - what every README must contain (Universal Sections, class-specific sections, three-tier model with `ROBOTS.md`)
- [Module Publishing](publishing.md) - `_test/` layout and the `package.json` shape it requires
- [Module Structure (JavaScript)](module-structure) - the factory pattern that test loaders mirror
