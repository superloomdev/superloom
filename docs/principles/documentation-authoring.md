...
- [Cross-Reference Integrity](#cross-reference-integrity)
  - [Anchor Targets Use the Site Form](#anchor-targets-use-the-site-form)
  - [On This Page Blocks Are Generated](#on-this-page-blocks-are-generated)
...
### Anchor Targets Use the Site Form

Anchor targets in markdown links (the `#anchor` fragment of a `[text](path)` link) are written in the **site's generated form**, not GitHub's. The two slugifiers disagree on punctuation, and the site is the surface the public reads. Obtain anchors from the built HTML (`website/DIST/**/*.html`) or the audit script (`__dev__/audits/anchor-audit.py`), never from GitHub's "copy link to heading" button, which yields the GitHub form.

VitePress does not validate heading fragments. `ignoreDeadLinks: []` being strict is not evidence that anchors are sound; it checks that linked files exist, not that the `#fragment` resolves. The anchor audit gate catches what the build cannot.

### On This Page Blocks Are Generated

The `## On This Page` block in each document is generated by `__dev__/audits/toc-generate.py` from the built HTML's heading ids. Do not hand-edit a generated block; the `--check` gate fails on the next build if a block has been touched. To change what appears in the block, change the headings in the document and re-run the generator.
