...
const ModuleName = {
  ...
  *********************************************************************/
  functionName: function (params) {
    ...
    // Second logical block - separated by ONE blank line
    return result;
...
- The one-line purpose comment under the marker explains what binds the functions together
...
- **No underscore prefix on parameters.** Never use `_param` to suppress ESLint's `no-unused-vars` warning
- Use an inline `// eslint-disable-line no-unused-vars` comment on the function signature instead

```javascript
// Correct - clean parameter name, inline directive when needed
const createInterface = function (Lib, CONFIG) { // eslint-disable-line no-unused-vars

// Wrong - underscore prefix pollutes the name and hides intent
const createInterface = function (Lib, _CONFIG) {
```
...
// Wrong - inline return objects are hard to read at a glance
...
Full rule with rationale, anti-patterns, and worked examples: [`error-handling.md`](error-handling.md).
...
Full module structure templates (Pattern 1 Singleton vs Pattern 2 Factory) live in [`module-structure.md`](module-structure.md) under "Helper Module Configuration Patterns".
