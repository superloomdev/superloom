...
**In practice:** stores are almost always factory (each instance closes over its own configuration). Adapters are more commonly singleton (stateless normalizers), but can be factory if a future use case requires per-instance adapter config.
