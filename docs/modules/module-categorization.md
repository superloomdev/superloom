...
*Reading this table:* a Class B module is allowed to use everything Class A is, plus Node built-ins. A Class C module is allowed to use everything Class B is, plus a self-hostable third-party service. And so on. F, G, and H are special cases: they cannot stand alone. Class F has two subtypes: **stores** (data persistence, named `-store-[backend]`) and **adapters** (everything else: runtimes, transports, integrations, named `-adapter-[name]`). Class H has the naming pattern `-ext-[framework]`. Both Class F subtypes share one internal shape identical to every other factory module (loader takes `(shared_libs, config)`, `Lib` picked by reference from the injected container, companion config/errors/validators files, factory `createInterface` with fixed slots); whether `createInterface` closes over per-instance state varies by adapter. See `module-structure-js.md` for the skeletons.
...
**Internal shape (one shape for both subtypes):** Every Class F module - store or adapter, stateful or stateless - uses the **standard injected-Lib factory shape**: the loader takes `(shared_libs, config)` like every other helper module, picks `Lib` by reference from the injected container, merges `config` over its companion config defaults, owns its own `ERRORS` and `Validators` (companion files, per the Universal Companion Files rule in `module-structure-js.md`), and returns a ready-to-use object via `createInterface(Lib, CONFIG, ERRORS, Validators)`. Statelessness only means `createInterface` closes over nothing beyond the four fixed slots. Two earlier shapes are deprecated: the module-scope singleton (`let Lib;` + `loader(shared_libs)`) and the self-built-Lib "fully-independent" factory (loader taking only `config`, constructing `Lib` from aliased peer dependencies - it duplicated Utils/Debug instances and broke dependency injection); modules still on either shape are brought to the standard shape during their unification pass.
...
11. **Wave 10 (Extension pattern):** `js-client-helper-styler` parent module and `js-client-helper-styler-ext-react` extension module. First extension-based module pair. Established the extension pattern: parent is pure JavaScript, extension adds framework bindings. Extension imports parent (not the other way around). Entry point `extension.js` (not `index.js`). Naming convention `*-ext-[framework]`. Classes G and H added to the categorization rubric.
