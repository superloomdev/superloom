...
*Reading this table:* a Class B module is allowed to use everything Class A is, plus Node built-ins. A Class C module is allowed to use everything Class B is, plus a self-hostable third-party service. And so on. F, G, and H are special cases: they cannot stand alone. Class F has two subtypes: **stores** (data persistence, named `-store-[backend]`) and **adapters** (everything else: runtimes, transports, integrations, named `-adapter-[name]`). Class H has the naming pattern `-ext-[framework]`. Both Class F subtypes share one fully-independent internal shape (own `Lib` from aliased peer dependencies, companion config/errors/validators files, factory `createInterface` with fixed slots); whether `createInterface` closes over per-instance state varies by adapter. See `module-structure-js.md` for the skeletons.
...
**Internal shape (one shape for both subtypes):** Every Class F module - store or adapter, stateful or stateless - is a **fully-independent factory**: the loader takes only `config`, builds its own `Lib` from aliased peer dependencies, merges `config` over its companion config defaults, owns its own `ERRORS` and `Validators` (companion files, per the Universal Companion Files rule in `module-structure-js.md`), and returns a ready-to-use object via `createInterface(Lib, CONFIG, ERRORS, Validators)`. Statelessness only means `createInterface` closes over nothing beyond the four fixed slots. The earlier module-scope singleton shape (`let Lib;` + `loader(shared_libs)`) for stateless adapters is deprecated; modules still on it are brought to the factory shape during their unification pass.
