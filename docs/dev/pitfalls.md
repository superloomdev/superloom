...
- [Rule Delivery](#rule-delivery)
  - [1. AI attribution trailers appear in commits despite a documented ban](#_1-ai-attribution-trailers-appear-in-commits-despite-a-documented-ban)
- [CI/CD Publishing](#ci-cd-publishing)
  - [1. `Bind for 127.0.0.1:NNNN failed: port is already allocated` in CI](#_1-bind-for-127-0-0-1-nnnn-failed-port-is-already-allocated-in-ci)
  - [2. `PROTOCOL_CONNECTION_LOST` only in CI](#_2-protocol-connection-lost-only-in-ci)
  - [3. Push triggers tests but not publish, even though `version` was bumped](#_3-push-triggers-tests-but-not-publish-even-though-version-was-bumped)
  - [4. CI runs tests for every module on every commit](#_4-ci-runs-tests-for-every-module-on-every-commit)
  - [5. `npm publish` runs but `409 You cannot publish over the previously published versions`](#_5-npm-publish-runs-but-409-you-cannot-publish-over-the-previously-published-versions)
  - [6. `403 Forbidden` on `npm publish`](#_6-403-forbidden-on-npm-publish)
  - [7. CI test fails with `npm error notarget No matching version found for @superloomdev/...`](#_7-ci-test-fails-with-npm-error-notarget-no-matching-version-found-for-superloomdev)
  - [8. CI test fails with `ERR_MODULE_NOT_FOUND` for a helper that exists in the repo](#_8-ci-test-fails-with-err-module-not-found-for-a-helper-that-exists-in-the-repo)
  - [9. Tests Fail in CI](#_9-tests-fail-in-ci)
  - [10. `Invalid workflow file: ... You have an error in your yaml syntax on line N`](#_10-invalid-workflow-file-you-have-an-error-in-your-yaml-syntax-on-line-n)
  - [11. `publish-*` jobs silently skip when an upstream `publish-*` is also skipped, breaking the test→publish chain](#_11-publish-jobs-silently-skip-when-an-upstream-publish-is-also-skipped-breaking-the-test→publish-chain)
  - [12. Main module and its adapters are both unpublished: bootstrap order](#_12-main-module-and-its-adapters-are-both-unpublished-bootstrap-order)
  - [14. `test-verify` CI fails with `Cannot find module '../stores/sqlite'` after stores/ was deleted](#_14-test-verify-ci-fails-with-cannot-find-module-stores-sqlite-after-stores-was-deleted)
  - [13. CI fails on lint after local tests pass: pre-publish checklist not followed](#_13-ci-fails-on-lint-after-local-tests-pass-pre-publish-checklist-not-followed)
  - [15. VitePress / Vue compiler crashes on bare angle-bracket placeholders in markdown](#_15-vitepress-vue-compiler-crashes-on-bare-angle-bracket-placeholders-in-markdown)
  - [16. Deleting a GitHub Packages package: org-scoped vs user-scoped endpoint](#_16-deleting-a-github-packages-package-org-scoped-vs-user-scoped-endpoint)
  - [17. `git add .` bundles unrelated modules into one commit](#_17-git-add-bundles-unrelated-modules-into-one-commit)
  - [23. Extension module's test job 404s on its base package: `needs` pointed at the base's test job, not its publish job](#_23-extension-module-s-test-job-404s-on-its-base-package-needs-pointed-at-the-base-s-test-job-not-its-publish-job)
...
- [Local Module Testing](#local-module-testing)
  ...
  - [3. Manually starting Docker before `npm test`](#_3-manually-starting-docker-before-npm-test)
  - [4. `node --test test.js` directly without `pretest`](#_4-node-test-test-js-directly-without-pretest)
  - [5. Stale container from a previous failed run](#_5-stale-container-from-a-previous-failed-run)
  - [6. Tests pass locally, fail in CI with `Connection lost: The server closed the connection`](#_6-tests-pass-locally-fail-in-ci-with-connection-lost-the-server-closed-the-connection)
  - [7. `test did not finish before its parent and was cancelled`](#_7-test-did-not-finish-before-its-parent-and-was-cancelled)
  - [8. `&& sleep 2` (or 5) in `pretest`](#_8-sleep-2-or-5-in-pretest)
  - [9. AI agents or scripts hanging on a multi-line shell command](#_9-ai-agents-or-scripts-hanging-on-a-multi-line-shell-command)
  - [10. AWS SDK test hangs ~1-2s, then `success: false` with no clear error](#_10-aws-sdk-test-hangs-1-2s-then-success-false-with-no-clear-error)
  - [11. MongoDB replica-set healthcheck reports healthy before the node is PRIMARY](#_11-mongodb-replica-set-healthcheck-reports-healthy-before-the-node-is-primary)
  - [12. `verify.generateAndStore` returns `COOLDOWN_ACTIVE` with `cooldown_seconds: 0` under concurrent calls](#_12-verify-generateandstore-returns-cooldown-active-with-cooldown-seconds-0-under-concurrent-calls)
  - [15. Repo-wide `ERR_MODULE_NOT_FOUND` after a "remove scope prefix" style commit](#_15-repo-wide-err-module-not-found-after-a-remove-scope-prefix-style-commit)
  - [16. Store contract method name drift between `logger.js` and test fixtures](#_16-store-contract-method-name-drift-between-logger-js-and-test-fixtures)
  - [17. Store contract method renames not propagated to adapter `_test/test.js` files: all adapter CI jobs fail](#_17-store-contract-method-renames-not-propagated-to-adapter-test-test-js-files-all-adapter-ci-jobs-fail)
  - [18. `contains()` substring matching causes parent modules to trigger when only adapters need publishing](#_18-contains-substring-matching-causes-parent-modules-to-trigger-when-only-adapters-need-publishing)
  - [19. Test loader uses relative source path instead of the `_test/package.json` alias](#_19-test-loader-uses-relative-source-path-instead-of-the-test-package-json-alias)
  - [20. Singleton pattern prevents test isolation](#_20-singleton-pattern-prevents-test-isolation)
  - [21. Stale `package-lock.json` causes wrong-version install after `file:` swap or version reset](#_21-stale-package-lock-json-causes-wrong-version-install-after-file-swap-or-version-reset)
  - [22. ESLint `no-unused-vars` on intentionally reserved module-scope variables](#_22-eslint-no-unused-vars-on-intentionally-reserved-module-scope-variables)
  - [23. CI fails after push - local tests ran against stale `node_modules`](#_23-ci-fails-after-push-local-tests-ran-against-stale-node-modules)
  - [24. `_data/` directory mixes dev scripts with generated data, violating the single-concern rule](#_24-data-directory-mixes-dev-scripts-with-generated-data-violating-the-single-concern-rule)
