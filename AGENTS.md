...
- **Injected slots are named for the capability, never the vendor:** `shared_libs.FontLoader` and `shared_libs.KeyValueStore` are correct; `shared_libs.ExpoFont` and `shared_libs.MMKV` are not. A vendor-named slot re-couples the module to that vendor through its own source text even though no import exists, which defeats the reason the dependency was injected. The rule binds module code, test loaders, host manifests, and doc examples equally. See `docs/languages/js/module-structure.md` - Class I Framework Module Deltas
