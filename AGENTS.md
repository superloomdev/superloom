...
**Driver injection (Class F adapters):** Backend drivers arrive via the `shared_libs` container, picked into `Lib` by reference - same mechanism as `Utils`/`Debug`. SQL stores take family-generic `Lib.SQL` (any dialect); non-interchangeable APIs take backend-specific `Lib.MongoDB` / `Lib.DynamoDB`. `config` carries plain data only (table/collection names, timeouts) - never live objects. Passing a driver through a `lib_*` config key (`lib_sql`, `lib_dynamodb`) is a **deprecated shape** - convert on sight. Source: `docs/modules/module-structure-js.md` -> "Driver injection and container key naming".
