...
>
> This is the only compiled `AGENTS.md`. A dependent repository carries a small repo-local one for its own commands and for policy this constitution defers to it, never a copy or symlink of this file.
>
> To change a rule:
...
- **Strict roster smoke tests run in the library's strictest contract mode:** undeclared utility/token reads throw during the roster walk; lenient rendering proves reachability only, not styling correctness. If no strict mode exists, a static contract audit compares every literal reference against the generated registry. See `docs/languages/js/testing-strategy.md` - Rules
...
- **RNW component libraries always consume the `native` theme projection:** `buildTheme(template, layers, 'native')` on every platform including web. RNW is itself the web projection; requesting `web` applies two projections and yields unit strings that React Native cannot consume on iOS or Android. See `docs/languages/js/client/theming.md` - Units and platforms
- **Token contract:** the Themer package owns the token contract (`Themer.getContract()`); a component system requires its tokens and declares the subset it requires and supports as exported data. `createSystem` calls `Themer.validateContract`; missing required tokens throw `TypeError`; no component contains a color literal or falls back from one token to another. See `docs/languages/js/client/theming.md` - The token contract, `docs/languages/js/client/components.md` - Theme Token Contract
- **Scheme versus brand layer:** a scheme is a complete token set that replaces the base template; a brand layer is a sparse overlay. Switching schemes replaces the template; switching a brand replaces one layer and never rebuilds the template. See `docs/languages/js/client/theming.md` - Themes, templates, layers, profiles
...
- **Theme token contract:** a component system requires its tokens from the Superloom token contract and declares the subset it requires and the subset it supports as exported data. `createSystem` calls `Themer.validateContract` with both lists: missing required tokens are one `TypeError` naming them all; unsupported provided tokens are one warning naming them all. No component source contains a color literal, reads a token by a name outside the contract, or falls back from one token to another. See `docs/languages/js/client/components.md` - Theme Token Contract
- **Motion interpretation lives in the component system:** the core Themer defines and validates motion token shapes (durations, bezier arrays, spring objects, segments); the component system interprets them into platform API calls via `parts/motion.js`. Even when a platform provides a native animation, the component system drives it through motion tokens, never relying on a platform default. See `docs/languages/js/client/theming.md` - Motion
...
- **Repository independence:** the constitution repo (superloom) never references dependent repos' workflows or internals; dependent repos reference superloom docs freely. A dependent repo's own `AGENTS.md` holds only what is true of that repo alone (its commands, its deferred policy) and never a copy of a compiled constitution rule. See `docs/ai/agent-configuration.md` - Repository Independence
...
- **Release policy is repository-local; normal SemVer is the default:** a source change earns a new version number unless the repository being published declares otherwise in its own `AGENTS.md`. A repository that instead pins a version and republishes by delete-then-push declares that there, flagged as a transitional pre-release convenience rather than a framework rule, and regenerates every consumer lockfile after each republish. The constitution never records which policy a dependent repo currently runs. See `docs/ai/agent-configuration.md` - Release Policy Is Repository-Local
- **Idempotency guards compare fingerprints, not names:** a guard whose job is to detect "already done" takes a fingerprint of the work as its input, never a name the work happens to carry, because a name cannot separate "nothing to do" from "the name was reused" and the guard then skips needed work while reporting success. Binds package publish, cache reads, migration ledgers, webhook replay, and build caches alike. See `docs/principles/engineering-philosophy.md` - Idempotency Guards Compare Fingerprints
...
- **Publish guards compare content, not version presence.** Both the detect gate and each `publish-*` job compare the `npm pack` shasum against the registry's `dist.shasum`: match skips, differ fails naming both shasums, absent publishes. A version-presence skip turns an unmoved version into a green run that ships nothing. The remedy for a mismatch follows the repository's release policy, which is normally to bump the version; delete-then-push is valid only where that repo's `AGENTS.md` documents it as its release mechanism, and deleting a version in a normally-bumping repo breaks consumer lockfiles. A version appearing in the registry listing is not proof the new content shipped, so post-publish verification compares the published shasum against `npm pack` for the commit and they must match. See `docs/dev/cicd-publishing.md` - The Publish Guard Compares Content, Not Version Presence and The Remedy Depends on the Release Policy, pitfalls entry 26
...
- **Anchor targets use the site's generated form**, not GitHub's. VitePress does not validate heading fragments; `ignoreDeadLinks: []` is not evidence. The anchor audit (`website/scripts/anchor-audit.py`) is the gate. `On This Page` blocks are generated by `website/scripts/toc-generate.py`, never hand-edited. See `docs/principles/documentation-authoring.md` - Anchor Targets Use the Site Form. Pitfalls entry V3
...
- **Never state a policy-dependent rule as permanent doctrine.** A constitution rule states the mechanism; a remedy that varies by repository is deferred to that repository's own `AGENTS.md`, with the unsurprising behavior as the default when nothing is declared. Journal a temporary convention as a condition in the pitfall's cause, never as a premise inside a rule. Test: for every "never X" in the constitution, name the repo where it is false. Pitfalls entry V5
- **Local CI parity:** a workflow policy gate has a local entry point that extracts the current gate body from the workflow file; do not hand-copy a regex and compare only gate counts. `git grep` is blind to untracked files, so a verifier fails on nonignored untracked files or the operator runs `git add -N` before trusting a clean result. Full local verification runs after the final edit and immediately before push. See `docs/dev/testing-local-modules.md` - Local CI parity, pitfalls entries 28 and 29
- **Active workflows use each external action's current stable major**, verified from the action's official release API; align every active workflow in one pass; exclude archived references and vendored dependencies. See `docs/dev/cicd-publishing.md` - Workflow Maintenance
- **RNW list windowing requires a bounded height:** an unbounded `FlatList`/`VirtualizedList` mounts the full roster, so a row-count assertion does not prove virtualization; a local-vs-CI row-count difference is a layout precondition, not permission to relax the threshold. See `docs/languages/js/client/rn-testing.md` - List windowing under React Native Web, pitfalls entry 30
...
project-superloom/                 (workspace root)
  ...
  codebase-rnw-components/         - RNW component library (published as @superloomdev/rnw-components)
