...
- **Identifier wire format:** prefer a parse that cannot be ambiguous over a constraint on the caller's data. Fixed-width right-anchored parsing allows any character in the leading segment; a reserved character is justified only when relaxing it would break a correctness property such as prefix-query isolation, never merely to make parsing easier. See `docs/languages/js/validation.md` - Identifier Format and Wire Parsing
