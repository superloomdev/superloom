...
- `package.json`: peer deps as caret ranges; every Superloom module consumed at runtime (including modules received only by injection through `shared_libs`) appears in `peerDependencies` with caret ranges (`^1.0.0`); a framework peer such as `react` or `react-native` uses `>=` because the host owns the version; `engines.node >= 24`; `publishConfig.registry` exactly `https://npm.pkg.github.com` (`dependencies.md`, `publishing.md`)
