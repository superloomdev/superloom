  ...
  *********************************************************************/
  validateConfig: function (store_config) {
    ...
    if (
      ...
      throw new Error('[js-server-helper-verify-store-sqlite] STORE_CONFIG must be an object');
    }
    ...
    if (
      ...
      throw new Error('[js-server-helper-verify-store-sqlite] STORE_CONFIG.table_name is required');
    }

    // lib_sql is required - the caller must inject the SQLite helper
    if (Lib.Utils.isNullOrUndefined(store_config.lib_sql)) {
      throw new Error('[js-server-helper-verify-store-sqlite] STORE_CONFIG.lib_sql is required (pass Lib.SQLite)');
    }
