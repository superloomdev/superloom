...
The factory validates `CONFIG.STORE_CONFIG` and returns a Store interface. Each factory call returns an independent Store; multiple Auth instances (different `ACTOR_TYPE` values, different tables) coexist in the same process and share the same `Lib.SQLite` driver helper if it is the same instance.
