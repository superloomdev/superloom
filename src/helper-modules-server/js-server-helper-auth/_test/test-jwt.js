// Info: JWT-mode tests for js-server-helper-auth (Phase 5).
// JWT logic is store-agnostic, so these tests run against the in-memory
// store. End-to-end refresh-token rotation against real backends is
// covered indirectly via the shared store suite (refresh_token_hash
// is a regular column there).
'use strict';

const { describe, it } = require('node:test');
const assert = require('node:assert/strict');

const Lib_factory = require('./loader')();
const Lib = Lib_factory.Lib;
...
describe('jwt-mode: verifyJwt rejects malformed / tampered / expired tokens', function () {
  ...
  it('rejects an empty / non-string jwt as INVALID_TOKEN', function () {
    ...
    assert.deepEqual(r1.error, TEST_ERRORS.INVALID_TOKEN);

    const r2 = auth.verifyJwt(instance, { jwt: null });
    assert.equal(r2.success, false);
    assert.deepEqual(r2.error, TEST_ERRORS.INVALID_TOKEN);
  ...
  it('rejects a token with the wrong signature', async function () {
    ...
    assert.deepEqual(verify.error, TEST_ERRORS.INVALID_TOKEN);
  ...
  it('rejects an expired token with SESSION_EXPIRED', async function () {
    ...
    assert.equal(verify.success, false);
    assert.deepEqual(verify.error, TEST_ERRORS.SESSION_EXPIRED);
  ...
  it('rejects a token from a sibling actor_type with ACTOR_TYPE_MISMATCH', async function () {
    ...
    assert.equal(verify.success, false);
    assert.deepEqual(verify.error, TEST_ERRORS.ACTOR_TYPE_MISMATCH);
...
describe('jwt-mode: refreshSessionJwt rotation flow', function () {
  ...
  it('invalidates the old refresh token after rotation (single-use)', async function () {
    ...
    assert.deepEqual(second.error, TEST_ERRORS.INVALID_TOKEN);
  ...
  it('rejects a malformed refresh token with INVALID_TOKEN', async function () {
    ...
    assert.deepEqual(r.error, TEST_ERRORS.INVALID_TOKEN);
  ...
  it('rejects refresh after session expires', async function () {
    ...
    assert.equal(r.success, false);
    assert.deepEqual(r.error, TEST_ERRORS.SESSION_EXPIRED);
...
describe('jwt-mode: db-mode APIs throw when called', function () {
