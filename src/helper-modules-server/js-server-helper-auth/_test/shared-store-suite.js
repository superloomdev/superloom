...
// (memory + sqlite + postgres + mysql + dynamodb + mongodb). Focuses
// on the public Auth API paths that talk to the store. Pure-helper
// tests stay in test.js so they only run once.
//
...
@param {Object} args.TEST_ERRORS - Domain error catalog used by buildAuth
...
module.exports = function runSharedStoreSuite (args) {
  ...
  describe(label + ': createSession + verifySession lifecycle', function () {
    ...
    it('rejects a forged auth_id (wrong secret)', async function () {
      ...
      assert.equal(result.error, TEST_ERRORS.INVALID_TOKEN);
    ...
    it('rejects a session after it expires', async function () {
      ...
      assert.equal(expired.error, TEST_ERRORS.SESSION_EXPIRED);
