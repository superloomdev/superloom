// Info: MySQL backend integration tests. Uses the shared store suite
// so every backend exercises identical behaviour. Requires the MySQL
// container from docker-compose.yml to be running on 127.0.0.1:3306.
'use strict';
...
const SqlDB = Lib.MySQL;
const TEST_TABLE = 'sessions_user_my';
...
before(async function () {
  ...
  // Drop first so schema changes land on each run
  ...
  if (result.success === false) {
    throw new Error('MySQL initializeSessionStore failed: ' + JSON.stringify(result.error));
  }
