...
`Lib.Auth.user.setupNewStore(instance)` should be called once at boot to create / migrate the underlying SQL table. Only SQL backends (sqlite, postgres, mysql) implement this method - on NoSQL backends (mongodb, dynamodb) it throws, and the schema must be provisioned out-of-band (IaC / one-shot script).
...
Every backend relies on `cleanupExpiredSessions` as its garbage-collection path. Schedule one daily job that sweeps every Auth instance:
...
- **SQL (Postgres / MySQL / SQLite)** — no native TTL. The cron is the only path. The `expires_at` index created by `setupNewStore(instance)` keeps the sweep an efficient range scan.
- **DynamoDB** — no GSI is required: the composite primary key (`PK=tenant_id`, `SK=actor_id#token_key`) serves every hot-path query. `cleanupExpiredSessions` is a table scan with a filter; provisioned scans are acceptable daily cost. You may enable native DynamoDB TTL on the `expires_at` attribute out-of-band (via IaC or AWS Console) — the auth module stores the value in the integer-seconds-since-epoch format that AWS requires. When native TTL is on, the cron becomes a belt-and-braces fallback.
- **MongoDB** — the auth module stores `expires_at` as an integer, not a `Date`, so MongoDB native TTL does not apply. The cron is required. Provision two indexes on the sessions collection out-of-band (the auth module does not create them): one on `prefix` (equality, serves `listSessionsByActor`) and one on `expires_at` (range, serves the cron).

## 6. NoSQL schema provisioning

`setupNewStore(instance)` only applies to SQL backends. For NoSQL, bring the table / collection + indexes up with IaC or a one-shot bootstrap script:

- **DynamoDB** — create the table with `AttributeDefinitions` covering `tenant_id` (S, PK) and `session_key` (S, SK). Optionally enable TTL on `expires_at`.
- **MongoDB** — create the collection (MongoDB auto-creates on first write, so this is optional) plus `db.sessions_user.createIndex({ prefix: 1 })` and `db.sessions_user.createIndex({ expires_at: 1 })`. Run once per Auth instance (the collection name comes from `STORE_CONFIG.collection_name`).

---

## 7. Error handling

The auth module returns its own internal `{ type, message }` error catalog on every failure path. Your error middleware inspects `error.type` and maps it to your project's HTTP status / code convention:

```js
// Map auth-module error.type -> HTTP status. Add cases as your app
// surfaces more error types from other modules.
const AUTH_ERROR_STATUS = {
  INVALID_TOKEN:        401,
  SESSION_EXPIRED:      401,
  ACTOR_TYPE_MISMATCH:  401,
  LIMIT_REACHED:        429,
  SERVICE_UNAVAILABLE:  503
};

// Inside any route that does `const r = await Lib.Auth.user.verifySession(...)`:
if (r.success === false) {
  const status = AUTH_ERROR_STATUS[r.error.type] || 500;
