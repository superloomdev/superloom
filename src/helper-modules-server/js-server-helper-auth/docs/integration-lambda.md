...
Don't put `cleanupExpiredSessions` inside an HTTP-triggered Lambda. Instead, use a separate scheduled Lambda triggered once per day by EventBridge:
...
- [ ] One-time call to `initializeSessionStore(instance)` is performed by a separate migration Lambda or in `serverless deploy --stage` post-hook.
...
- [ ] EventBridge schedule triggers the cleanup Lambda for SQL backends.
