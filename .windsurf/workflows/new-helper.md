...
Follow all coding standards from `docs/foundations/code-formatting-js.md`:
...
- **Badges** - exactly 3 header badges for every module (Test + License + Node.js). Test badge uses GitHub's native endpoint pointing at `ci-helper-modules.yml`. Service-dependent modules add an Integration Tests badge inside the `## Testing` table, not the header. Templates in `docs/testing/module-testing.md`
...
- **Testing** - lean section near the end (status table + 3-line run command + link to ops guide). Full setup steps live in `_test/ops/` only. See template in `docs/testing/module-testing.md`
...
Every key in `.env.dev` must also exist in `.env.integration`. Never skip a key in one file. Full rules in `docs/testing/module-testing.md` → "Environment Variable Registration".
